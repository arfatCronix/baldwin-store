...
export class FeaturedProducts {
  ...
  async buildHTML() {
    ...
    buildCardHtml(product, index) {
        const { node } = product;
        return `
      <div data-feat-product-slide class="productCarousel-slide data">
        <article
          class="card"
        ...
        <figure class="card-figure">
          ...
          <figcaption class="card-figcaption">
            <div class="card-figcaption-body">
              <button type="button" class="button button--primary button--small card-figcaption-button quickview"
                      data-event-type="product-click" data-product-id="${node.entityId}">Quick view</button>
            </div>
          </figcaption>
        </figure>
        <div class="card-body">
          ${node.brand ? '<p class="card-text" data-test-info-type="brandName">${node.brand}</p>' : ''}
          <h3 class="card-title">
